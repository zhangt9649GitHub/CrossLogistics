$(document).ready(function(){
	...
	layui.use(['layer', 'form','formSelects'], function(){
        ...
        form.on('submit(add)', function(data){
            ...
            axios.post("/staffGroupAccess/editStaffGroup", data.field)
       ...
       function editInfo() {
           $.ajax({
               url: '/staffGroupAccess/selectEditStaffGroup',
               dataType: 'JSON',
               type: 'post',
               data: {'staffGroupId':$('input[name = "staffGroupId"]').val()},
               success: function (data) {
                   if(data.data != null){
                       var res = data.data;
                       $('input[name = "staffGroupName"]').val(res.staffGroupName);
                       formSelects.value('select1', res.saIds);
                   }
               },
               error: function (data) {
