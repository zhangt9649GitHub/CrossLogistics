...
$(document).ready(function(){
    ...
    layui.use(['layer', 'form','table'], function(){
        ...
        form.on('submit(add)',function(data){
            ...
            axios.post("/car/updateCar", data.field)
					...
					if(data.code == 0){
						if(data.data != null){
                            ...
                            $('select[name = "singaporeAreaId"] option').each(function (i,item) {
                                if($(item).val() == editData.singaporeAreaId){
                            ...
                            $.each(res,function (i,n) {
                                if(n.singaporeAreaId == editData.singaporeAreaId){
                                    var singaporePoints = n.singaporePoints;

                                    var latlngArray = [];
                                    $.each(n.singaporePoints,function (i,n) {
                                        latlngArray.push({lat:Number(n.lat),lng:Number(n.lng)})
                                    });
                            ...
                            // 集结点
                            if(data.data.singaporeAreaId != null){
                                $.ajax({
                                    url: '/truckTask/selectRallyPointBySA',
                                    data: {'singaporeAreaId': editData.singaporeAreaId},
                                    dataType: 'JSON',
                                    async: false,
                                    success: function (data) {
                                        if(data.message == '获取成功'){
                            ...
                            $('select[name = "rallyPointId"] option').each(function (i,item) {
                                if($(item).val() == editData.rallyPointId){
                            ...
                            // 车锁
                            $('select[name = "carLockId"]').append('<option value="'+editData.carLockId+'" selected>'+editData.carLockNumber+'</option>');
                            // 车锁
                            $('select[name = "carLockBoxId"]').append('<option value="'+editData.carLockBoxId+'" selected>'+editData.carLockBoxNumber+'</option>');
                            form.render('select');
						}
					}
