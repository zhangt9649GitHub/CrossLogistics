$(document).ready(function(){
    ...
    layui.use(['layer', 'form','table'], function(){
        ...
        form.on('submit(add)', function(data){
            ...
            axios.post("/copyWriter/updateCopyWriter", data.field)
    ...
    $.ajax({
        ...
        data: {'cwId': $('input[name = "cwId"]').val()},
        dataType: 'JSON',
        success: function (data) {
            if(data.data != null){null
                $('input[name = "cwName"]').val(data.data.cwName);

                $('select[name = "language"] option').each(function(i,item){
                    if($(item).val() == data.data.language){
                        $(item).prop('selected',true);

                    }
                });

                $('#HandoverCompany').val(data.data.type);
                if(data.data.type == '转运须知' || data.data.type == '转运公司介绍'){
                    $('.tubiao b').text('');
                }

                $('#preview img').attr('src',data.data.icon);
