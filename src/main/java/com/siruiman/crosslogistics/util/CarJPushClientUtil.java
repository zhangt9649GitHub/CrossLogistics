...
public class CarJPushClientUtil {
    //别名：测试: CarDebug+用户编号  发布: CarRelease+用户编号
    private static final Logger log = LoggerFactory.getLogger(CarJPushClientUtil.class);
    private static Boolean AF = true; //1.测试 false 2.生产 true
    //极光后台管理系统申请
    //小车端
    private static final String appKey = "cd9a8d0f56f4ddd6a9ba50af";
    private static final String masterSecret = "249affa3aaf249bbfc51ee1b";
     ...
     * @return 0推送失败，1推送成功
     */
    public static int sendCarToRegistrationId(String registrationId, String notification_title, String msg_title, String msg_content, String extrasparam, String extrasparam2) {
        int result = 0;
        try {
            PushPayload pushPayload = CarJPushClientUtil.buildPushObject_all_registrationId_alertWithTitle(registrationId, notification_title, msg_title, msg_content, extrasparam, extrasparam2);
    ...
    public static int sendToAll(String title, String content) {
        int result = 0;
        try {
            PushPayload pushPayload = CarJPushClientUtil.buildPushObject_android_and_ios(title, content);
            log.info("推送给设备标识参数的用户" + pushPayload);
   ...
   /* public static int sendToAll( String title, String content,String extrasparam, String extrasparam2) {
     ...
     * @param alert              推送的内容
     */
    public static void pushCarMsg(String type, String notification_title, String value, String alert) {
        PushPayload.Builder builder = PushPayload.newBuilder();
        if (AF) {
            StringBuilder sb = new StringBuilder(value);//构造一个StringBuilder对象
            sb.insert(3, "Release");//在指定的位置3，插入指定的字符串
            value = sb.toString();
        } else {
            StringBuilder sb = new StringBuilder(value);//构造一个StringBuilder对象
            sb.insert(3, "Debug");//在指定的位置3，插入指定的字符串
            value = sb.toString();
        }
